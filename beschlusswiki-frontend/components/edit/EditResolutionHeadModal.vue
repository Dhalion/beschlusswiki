    ...
    <div>
        ...
        <UModal v-model="isOpen">
            <UCard :ui="{ ring: '', divide: 'divide-y divide-gray-100 dark:divide-gray-800' }">
                ...
                <!--* BODY *-->
                <UForm class="grid grid-rows-3 grid-cols-2 items-start gap-x-10 gap-y-3" :state="state" :validate="validate"
                    @submit="submit">
                    <UFormGroup label="Titel" name="title" class="col-span-2">
                        <UInput placeholder="Titel" v-model="state.title" />
                    </UFormGroup>
                    <UFormGroup label="Tag" name="tag">
                        <UInput placeholder="Tag" v-model="state.tag" />
                    </UFormGroup>
                    <UFormGroup label="Jahr" name="year">
                        <UInput placeholder="Jahr" v-model="state.year" />
                    </UFormGroup>


                    <UButton type="submit" class="col-start-2 h-2/3 place-self-end self-center" :disabled="!isInputValid">
                        Speichern</UButton>
                </UForm>
...
const validate = (state) => {
    ...
    if (state.year && isNaN(parseInt(state.year))) errors.push({ path: "year", message: "Jahr ist keine Zahl" });
