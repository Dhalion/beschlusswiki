        ...
        <UForm :state="state" :validate="validate" class="flex flex-col gap-3" @submit="handleSubmit">
          ...
          <UFormGroup label="Rollen" name="roles">
            <div class="flex flex-row pt-1 pb-2">
              ...
              <AdminRoleBadgeView v-model:roles="state.roles" />
            </div>
          </UFormGroup>
          ...
          <div class="flex flex-row justify-end gap-2">
            ...
            <UButton label="Erstellen" variant="solid" color="primary" icon="i-heroicons-check-badge" type="submit" />
          </div>

          <UAlert title="Fehler beim erstellen des Nutzers" :description="errorMessage"
            icon="i-heroicons-information-circle" color="red" variant="subtle" v-if="errorMessage">
...
const UserRolesItems = [[
  { label: 'Admin', click: () => addUserRole(UserRoles.Admin) },
  { label: 'Editor', click: () => addUserRole(UserRoles.Editor) },
  { label: 'Contributor', click: () => addUserRole(UserRoles.Contributor) },
]];
...
async function handleSubmit(event: FormSubmitEvent<IUserInput>) {
  ...
  if (error.value) {
    errorMessage.value = `${error.value?.statusCode}: ${error.value?.statusMessage}`;
    toast.add({
      title: "Fehler beim Erstellen des Benutzers",
      description: `Fehler: ${error.value.statusMessage}`,
